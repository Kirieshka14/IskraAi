...
function writeDigit(i:number,value:string){const clean=value.replace(/\D/g,"");
if(clean.length>1){const next=EMPTY.map((_,k)=>clean[k]??"" );setDigits(next);const focus=Math.min(clean.length,5);boxRefs.current[focus]?.focus();setActiveBox(focus);return}
setDigits(prev=>{const next=[...prev];next[i]=clean;if(clean&&i<5){boxRefs.current[i+1]?.focus();setActiveBox(i+1)}return next})}
function boxKeyDown(i:number,e:KeyboardEvent<HTMLInputElement>){if(e.key==="Backspace"&&!digits[i]&&i>0){setDigits(prev=>{const next=[...prev];next[i-1]="";return next});boxRefs.current[i-1]?.focus();setActiveBox(i-1)}}
...
function finish(r:AuthSession){storeSession({...r.session,userId:r.user.id,email:r.user.email});router.replace((()=>{const next=new URLSearchParams(window.location.search).get("next");return next?.startsWith("/")?next:"/"})())}
function switchMode(x:Mode){setSlide(x==="login"?"right":"left");setMode(x);setStep("email");setError(null);setAnimKey(k=>k+1);ref.current?.reset()}
const formClass=`mt-6 grid gap-4 ${slide==="left"?"auth-form-from-left":slide==="right"?"auth-form-from-right":"auth-form-animated"}`;
return <main className="auth-shell"><section className="auth-card"><div className="auth-brand"><BrandMark className="h-16 w-16"/><span>IskraAi</span></div>
{step==="code"?<>
<h1 className="display text-center text-3xl font-semibold text-white">Код из письма</h1>
<p className="mt-3 text-center text-sm text-stone-400">Отправили 6 цифр на {email}. Код действует 10 минут.</p>
<div className="otp-boxes">{EMPTY.map((_,i)=>(<input key={i} ref={el=>{boxRefs.current[i]=el}}className={"otp-box"+(i===activeBox?" is-active":"")}inputMode="numeric"autoComplete="one-time-code"maxLength={6}value={digits[i]}onFocus={()=>setActiveBox(i)}onChange={(e:ChangeEvent<HTMLInputElement>)=>writeDigit(i,e.target.value)}onKeyDown={(e:KeyboardEvent<HTMLInputElement>)=>boxKeyDown(i,e)}onPaste={boxPaste}disabled={busy}aria-label={`Цифра ${i+1}`}/>))}</div>
</>:<>
<h1 className="display text-center text-3xl font-semibold text-white">{mode==="register"?"Создай аккаунт":"С возвращением"}</h1>
<p className="mt-3 text-center text-sm text-stone-400">{mode==="register"?"Персонажи, диалоги и истории ждут":"Войди, чтобы продолжить общение"}</p>
<div className="auth-tabs"><span aria-hidden="true" className={"auth-tabs-pill"+(mode==="login"?" is-login":"")}></span><button type="button" onClick={()=>switchMode("register")} aria-selected={mode==="register"}>Регистрация</button><button type="button" onClick={()=>switchMode("login")} aria-selected={mode==="login"}>Вход</button></div>
</>}
<form key={animKey} className={formClass} onSubmit={submit}>
{step==="email"?<>
{mode==="register"&&<Field label="Имя или псевдоним"><input required maxLength={80} value={name} onChange={e=>setName(e.target.value)} className={`${inputClass} auth-input`} autoComplete="nickname"/></Field>}
