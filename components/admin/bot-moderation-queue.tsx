  ...
  return <div className="grid gap-5">
    ...
    {bots.map(bot => <article key={bot.id} className="rounded-2xl border border-stone-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="text-xs text-stone-500 sm:text-right"><div>{bot.author_display_name || "Без имени"}</div><div className="font-mono">{bot.author_id}</div></div>
      </div>
      <section className="mt-5 grid gap-4">
        ...
        <div><h3 className="text-xs font-bold uppercase tracking-wide text-stone-500">System prompt</h3><pre className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap rounded-xl bg-stone-950 p-4 text-xs leading-5 text-stone-100">{bot.system_prompt}</pre></div>
      </section>
      <div className="mt-5 grid gap-3 border-t border-stone-100 pt-5 sm:grid-cols-[1fr_auto_auto] sm:items-end">
        ...
        <button disabled={busyId !== null} onClick={() => moderate(bot.id, "reject")} className="h-11 rounded-xl border border-red-300 px-5 text-sm font-semibold text-red-700 hover:bg-red-50 disabled:opacity-50">Отклонить</button>
        <button disabled={busyId !== null} onClick={() => moderate(bot.id, "approve")} className="h-11 rounded-xl bg-moss px-5 text-sm font-semibold text-white hover:opacity-90 disabled:opacity-50">Одобрить</button>
      </div>
    </article>)}
  </div>;
}
